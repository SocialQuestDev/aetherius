...
JoinGamePacket::JoinGamePacket(std::shared_ptr<Player> player, World& world) : player(player), world(world) {}
...
void JoinGamePacket::write(PacketBuffer& buffer) {
    ...
    player->setGamemode(Gamemode(worldConfig["gamemode"].value_or(0)));
